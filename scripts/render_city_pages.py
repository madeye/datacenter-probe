...
<body>
  ...
  <div class="neatline">
    ...
    <span class="hide-narrow">原核查 · 2026-08-27</span>
  </div>
  <header class="mast">
...
def imagery_note():
    manifest = ROOT / "imagery.json"
    if not manifest.exists():
        return ""
    data = json.loads(manifest.read_text())
    date = data["fetched_at"][:10]
    return (f'<p class="imagery-note">影像获取：{date} · Esri World Imagery。'
            '获取日期不是拍摄日期；文字与框线沿用 2026-08 核查，建设状态需结合新图复核。</p>')
...
def main():
    ...
    for p in PAGES:
        ...
        html = HEAD.format(**p, imagery_note=imagery_note())
        out.write_text(html)
